...
local plugins_count = vim.fn.len(vim.fn.globpath(
  vim.fn.stdpath("data") ..
  "/site/pack/packer/start", "*", 0, 1)) +
    vim.fn
    .len(
      vim.fn
      .globpath(
        vim.fn.stdpath("data") ..
        "/site/pack/packer/opt", "*", 0, 1))
local plugins_loaded = vim.fn.len(vim.fn.globpath(
  vim.fn.stdpath("data") ..
  "/site/pack/packer/start", "*", 0, 1))
...
return {
  layout = {
    { type = "padding", val = 2 }, {
      ...
      opts = { spacing = 1 }
    }, startify.section.mru, startify.section.mru_cwd, {
      type = "text",
      val = {
        " AstroNvim loaded " .. plugins_loaded .. " and has " ..
