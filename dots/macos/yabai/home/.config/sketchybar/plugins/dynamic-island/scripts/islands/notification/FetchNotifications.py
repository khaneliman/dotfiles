    ...
    try:
        ...
        if curNotifCount > lastNotifCount:
            ...
            for row in getDetails:
                ...
                home = expanduser("~")
                queueIslandScript = "~/.config/sketchybar/plugins/dynamic-island/scripts/queue_island.sh"
                subprocess.check_call([expanduser(queueIslandScript), \
                                          "notifications;", \
                                          "3;", \
                                          "{}/.config/sketchybar/plugins/dynamic-island/scripts/islands/notification/notification_island.sh {}|{}|{}|{};".format(home, title, subtitle, message, appId), \
                                          "{}/.config/sketchybar/plugins/dynamic-island/scripts/islands/notification/reset.sh;".format(home), \
                                          "1.2" \
                                      ])
